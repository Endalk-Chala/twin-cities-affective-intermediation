...
AFFECT={'fear_intensity':'Fear','anxiety_uncertainty_intensity':'Anxiety / uncertainty','anger_intensity':'Anger','moral_outrage_intensity':'Moral outrage','grief_sadness_intensity':'Grief / sadness','solidarity_intensity':'Solidarity','care_compassion_intensity':'Care / compassion','empathy_intensity':'Empathy','hope_intensity':'Hope','gratitude_intensity':'Gratitude','pride_intensity':'Pride','reassurance_intensity':'Reassurance','defiance_intensity':'Defiance','urgency_emotion_intensity':'Urgency'}
...
# 1) FULL CODED CORPUS -> ORGANIZATION-AFFECT EDGES
...
# 2) ALL EXISTING VERIFIED MEDIA UPTAKE BATCHES -> ORGANIZATION-MEDIA EDGES
parts=[]
...
if len(m):
    ...
    if CORR.exists():
        q=pd.read_csv(CORR,dtype=str,keep_default_na=False); pairs=set(zip(q.source_batch,q.legacy_news_item_id))
        if 'news_item_id' in single:
            single=single.loc[~single.apply(lambda r:(s(r.get('source_batch','')),s(r.get('news_item_id',''))) in pairs,axis=1)]
        q2=pd.DataFrame({'source_batch':q.source_batch,'news_item_id':q.legacy_news_item_id,'date':q.date,'outlet':q.outlet,'headline':q.headline,'url':q.url,'org_id':q.org_id,'organization':q.organization,'organization_visibility':q.organization_visibility,'organization_quoted':q.organization_quoted,'spokesperson':q.spokesperson,'source_function':q.source_function,'matched_org_item_id':q.matched_org_item_id,'match_confidence':q.match_confidence})
...
# 3) PRESERVE TRANSFORMATION AUDIT VERBATIM AS LINKED LAYER
trans_rows=0
if TRANS.exists():
...
# 4) NETWORK NODES: retain corpus organizations AND media-only organizations.
meta={s(r.org_id):{'organization':s(r.organization),'observed_items':int(r.observed_items),'corpus_member':1} for _,r in org_counts.iterrows()}
for _,r in om[['org_id','organization']].drop_duplicates().iterrows():
    oid=s(r.org_id)
    if oid not in meta: meta[oid]={'organization':s(r.organization),'observed_items':0,'corpus_member':0}

G=nx.Graph()
for a in sorted(oa.affect.unique()):G.add_node('A::'+a,label=a,layer='affect',node_type='affect')
for oid,d in meta.items():G.add_node('O::'+oid,label=d['organization'],layer='organization',node_type='organization',observed_items=d['observed_items'],corpus_member=d['corpus_member'])
for x in sorted(om.outlet.unique()):G.add_node('M::'+x,label=x,layer='media',node_type='media')
...
an=sorted([n for n,d in G.nodes(data=True) if d.get('layer')=='affect'],key=lambda n:G.degree(n,weight='raw_weight'),reverse=True)
on=sorted([n for n,d in G.nodes(data=True) if d.get('layer')=='organization'],key=lambda n:G.degree(n,weight='raw_weight'),reverse=True)
mn=sorted([n for n,d in G.nodes(data=True) if d.get('layer')=='media'],key=lambda n:G.degree(n,weight='raw_weight'),reverse=True)
...
def draw(presentation=False):
    fig,ax=plt.subplots(figsize=(18,max(12,len(on)*.29))); cut=oa.normalized_intensity_per_item.quantile(.5)
    ...
    corpus_orgs=[n for n in on if G.nodes[n].get('corpus_member')==1]; media_only=[n for n in on if G.nodes[n].get('corpus_member')==0]
    nx.draw_networkx_nodes(G,pos,nodelist=corpus_orgs,node_shape='o',node_size=[osz[n] for n in corpus_orgs],linewidths=.6,edgecolors='black',ax=ax)
    if media_only:nx.draw_networkx_nodes(G,pos,nodelist=media_only,node_shape='^',node_size=[osz[n] for n in media_only],linewidths=.7,edgecolors='black',ax=ax)
    ...
    for n in on:
        x,y=pos[n];ax.text(x,y,G.nodes[n]['label'],ha='center',va='center',fontsize=7.2 if omstr.get(n,0)>0 else 6.1,fontweight='bold' if omstr.get(n,0)>=4 else 'normal',bbox=dict(boxstyle='round,pad=.12',facecolor='white',edgecolor='none',alpha=.78),zorder=4)
    ...
    ax.set_title('Affective Circulation Network — Presentation View' if presentation else 'Affective Circulation Network',fontsize=20,fontweight='bold',pad=18)
    if presentation:ax.text(.5,1.005,'All nodes and all media ties retained; weaker affect ties remain visible in the background.',transform=ax.transAxes,ha='center',fontsize=10)
        ...
        ax.text(.5,1.005,f'All coded nonprofit communication (N={len(c)}) plus all verified media-uptake relations in the repository',transform=ax.transAxes,ha='center',fontsize=10)
        ax.text(.5,-.012,'Diamonds = affect; circles = coded-corpus organizations; triangles = media-only organizations; squares = news outlets. Frame transformation remains a separate linked layer.',transform=ax.transAxes,ha='center',fontsize=8.5)
...
['coded_communication_items',len(c)],['coded_corpus_organizations',c.org_id.nunique()],['all_organizations_in_integrated_network',len(on)],['media_only_organizations',sum(1 for n in on if G.nodes[n].get('corpus_member')==0)],['organization_affect_edges',len(oa)],['deduplicated_verified_media_relations',len(mr)],['organization_media_edges',len(om)],['media_outlets',om.outlet.nunique() if len(om) else 0],['affect_nodes',len(an)],['transformation_audit_physical_rows',trans_rows]],columns=['metric','value'])
